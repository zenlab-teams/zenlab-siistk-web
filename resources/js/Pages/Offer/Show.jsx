...
import { TbCheck, TbPhoto, TbPlus, TbShoppingCart, TbTrash, TbX } from "react-icons/tb";
...
const OfferShow = ({ flash, offer, customers }) => {
    ...
    const handleApproveRecord = (recordId) => {
        router.post(route("offer.record.approve", [offer.id, recordId]));
    };

    const handleRejectRecord = (recordId) => {
        if (!window.confirm("Tolak laporan ini?")) {
            return;
        }

        router.patch(route("offer.record.reject", [offer.id, recordId]));
    };

    const handleCompleteOffer = () => {
        if (!window.confirm("Selesaikan offer ini? Qty tidak terjual akan dikembalikan ke stock.")) {
            return;
        }

        router.patch(route("offer.complete", offer.id));
    };

    const handleRejectOffer = () => {
        if (!window.confirm("Batalkan offer ini? Semua stock yang dibawa akan dikembalikan.")) {
            return;
        }

        router.patch(route("offer.reject", offer.id));
    };
                                        ...
                                        {offer.status === "active" && record.status === "pending" && (
                                            <>
                                                <button
                                                    ...
                                                    className="text-slate-500 hover:text-emerald-500 transition-all"
